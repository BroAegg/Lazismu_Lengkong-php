...
<div class="fixed bottom-0 left-0 right-0 bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-[60] px-6 py-3 lg:hidden flex justify-between items-center border-t border-gray-100 pb-safe">
    <a href="{{ route('beranda') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('beranda') ? 'text-primary-500' : 'text-gray-400' }} group hover:text-primary-500 transition-colors">
    ...
    {{-- Center Donasi Button (Elevated) --}}
    <div class="relative -top-8">
        <a href="{{ route('donasi') }}" class="flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r from-primary-500 to-accent-500 text-white shadow-lg shadow-orange-200 hover:shadow-orange-glow transform hover:-translate-y-1 transition-all duration-300 ring-4 ring-white border border-orange-100">
            <i class="fas fa-heart text-2xl"></i>
        </a>
        <span class="absolute -bottom-6 w-full text-center text-[0.65rem] font-bold text-gray-800">Donasi</span>
    </div>

    <a href="{{ route('kalkulator') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('kalkulator') ? 'text-primary-500' : 'text-gray-400' }} group hover:text-primary-500 transition-colors">
    ...
    <a href="{{ auth()->check() ? route('akun') : route('login') }}" class="flex flex-col items-center gap-1 {{ request()->routeIs('akun') ? 'text-primary-500' : 'text-gray-400' }} group hover:text-primary-500 transition-colors">
